        ...
        <div class="modal-dialog" role="document">
            ...
            <div class="modal-body">
                <form action="<?= base_url()?>kelas/edit_program" method="post">
                    <input type="hidden" name="id_kelas" id="id-edit-program">
...
<!-- Modal pembinaan -->
    <div class="modal fade" id="modalPembinaan" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="modalPembinaanLabel"></h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
                ...
                <form action="<?= base_url()?>kelas/add_kbm_pembinaan" method="post">
                    ...
                    <div class="d-flex justify-content-end">
                        <button type="button" class="btn btn-sm btn-secondary mt-2 mr-2" data-dismiss="modal">Tutup</button>
                        <input type="submit" value="Simpan" id="btnPembinaan" class="btn btn-sm btn-primary mt-2">
                    </div>
                </form>
            </div>
            </div>
...
<!-- Modal badal -->
    <div class="modal fade" id="modalBadalPembinaan" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="modalBadalPembinaanLabel">Badal Pembinaan</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
                ...
                <form action="<?= base_url()?>kelas/add_badal_pembinaan" method="post">
                    <input type="hidden" name="id_kelas" id="id-kelas-badal-pembinaan">
                    <div class="form-group">
                        <label for="koor-badal">Koordinator</label>
                        <input type="text" name="koor" id="koor-badal" value="LKP TAR-Q" class="form-control form-control-sm" readonly>
                    </div>
                    <div class="form-group">
                        <label for="tgl-badal">Tgl Badal</label>
                        <input type="date" name="tgl" id="tgl-badal" class="form-control form-control-sm" required>
...
<!-- Modal KBM -->
        ...
        <div class="container">
            ...
            <?php endif; ?>
            <div class="row">
                <?php if(COUNT($kelas + $kelas_pembinaan) != 0):?>
                                ...
                                <li class="list-group-item d-flex justify-content-between">
                                    <span>
                                        ...
                                        <?php if($kelas['program'] == "Tahfidz Anak" || $kelas['program'] == "Tahfidz Remaja" || $kelas['program'] == "Tahfidz Dewasa") :?>
                                            <a href="<?= base_url()?>kelas/tahfidz/<?= md5($kelas['id_kelas'])?>" class="btn btn-sm btn-warning"><i class="fa fa-file"></i></a>
                                        <?php elseif($kelas['program'] == "Pra Tahsin 1" || $kelas['program'] == "Pra Tahsin 2" || $kelas['program'] == "Pra Tahsin 3" || $kelas['program'] == "Tahsin 1" || $kelas['program'] == "Tahsin 2" || $kelas['program'] == "Tahsin 3" || $kelas['program'] == "Tahsin 4" || $kelas['program'] == "Tahsin Lanjutan") : ?>
                                            <a href="<?= base_url()?>kelas/tahsin/<?= md5($kelas['id_kelas'])?>" class="btn btn-sm btn-warning"><i class="fa fa-file"></i></a>
                                        <?php elseif($kelas['program'] == "Bahasa Arab 1" || $kelas['program'] == "Bahasa Arab 2" || $kelas['program'] == "Bahasa Arab 3" || $kelas['program'] == "Bahasa Arab 4" || $kelas['program'] == "Bahasa Arab Lanjutan") : ?>
                                            <a href="<?= base_url()?>kelas/b_arab/<?= md5($kelas['id_kelas'])?>" class="btn btn-sm btn-warning"><i class="fa fa-file"></i></a>
                                        <?php endif;?>
                                    </span>
                    ...
                    <?php endforeach;?>
                    <?php foreach ($kelas_pembinaan as $kelas) :?>
                        <div class="col-12 col-md-3 mb-2">
                            <ul class="list-group shadow">
...
<script>
    ...
    // modal badal pembinaan
        $(".modal-badal-pembinaan").click(function(){
            let id = $(this).data("id");
            $("#id-kelas-badal-pembinaan").val(id)
        })
    // modal badal pembinaan
    ...
    // modal presensi pembinaan
        $(".modal-pembinaan").click(function(){
            $("#info-presensi").show();
            ...
            $("#modalPembinaanLabel").html(data[1]);
            ...
            $.ajax({
                ...
                success: function(data){
                    let html = `<ul class="list-group"><li class="list-group-item list-group-item-info"><i class="fa fa-users"></i> List Peserta</li>`;
                    for (let i = 0; i < data.length; i++) {
                        html += `<li class="list-group-item" style="text-transform: capitalize"><input type="checkbox" value="`+data[i].nip+`"name="kpq[]" id="kpq`+i+`">
                    <label for="kpq`+i+`">`+data[i].nama_kpq.toLowerCase()+`</label></li>`
                    }
                    html += `</ul>`;
    ...
    // modal kbm
        ...
        $(".modal-kbm-pembinaan").click(function(){
            let id = $(this).data("id");
            console.log(id)
                ...
                success: function(data){
                    ...
                    if (data.length != 0){
                        for (let i = 0; i < data.length; i++) {
                            ...
                            if(data[i].badal == ""){
                                html += `<ul class="list-group mb-2"><li class="list-group-item list-group-item-info d-flex justify-content-between">`+data[i].kbm.hari+`[`+data[i].kbm.jam+`] `+tgl+`<span><a href="<?= base_url()?>kelas/delete_kbm_pembinaan/`+data[i].kbm.id_kbm+`" onclick="return confirm('Anda yakin akan menghapus KBM ini?')"><i class="fa fa-trash-alt"></i></a></span></li>`;
                            } else {
                                html += `<ul class="list-group mb-2"><li class="list-group-item list-group-item-danger d-flex justify-content-between">`+data[i].kbm.hari+`[`+data[i].kbm.jam+`] `+tgl+`<span><a href="<?= base_url()?>kelas/delete_kbm_pembinaan/`+data[i].kbm.id_kbm+`" onclick="return confirm('Anda yakin akan menghapus KBM ini?')"><i class="fa fa-trash-alt"></i></a></span></li>`;
                                html += `<li class="list-group-item">Dibadal : `+data[i].badal.nama_kpq+`</li>`;
                            }

                            total = parseInt(data[i].peserta_hadir + data[i].peserta_tidak_hadir)
                            if(total != 0){
                                html += `<li class="list-group-item">Peserta [`+total+`] = hadir [`+data[i].peserta_hadir+`] tidak hadir [`+data[i].peserta_tidak_hadir+`] </li>`
                            } else {
                                html += `<li class="list-group-item">
                                            <div class="alert alert-warning" role="alert">
                                                <i class="fa fa-info-circle mr-1 text-warning"></i> KBM belum direkap
